...
testAgentCreation = testGroup "Agent Creation"
  [ testCase "Create agent with name, description, and model" $ do
      ...
      let result = createAgent "test_agent" model (Just "Test agent description")
      case result of
        Right agent -> do
  ...
  , testCase "Agent name cannot be empty" $ do
      let model = createModel "gpt-4" OpenAI
      let result = createAgent "" model Nothing
      case result of
  ...
  , testCase "Agent name uniqueness validation" $ do
      ...
      let model = createModel "gpt-4" OpenAI
      let agent1 = createAgent "duplicate_name" model Nothing
      let agent2 = createAgent "duplicate_name" model Nothing
...
testModelConfiguration = testGroup "Model Configuration"
  ...
  , testCase "Model accessors work correctly" $ do
      ...
      agentModel (case createAgent "test" model Nothing of Right a -> a; Left _ -> error "Should not fail") @?= model
  ]
...
testAgentAccessors = testGroup "Agent Accessors"
  [ testCase "agentName accessor" $ do
      let model = createModel "gpt-4" OpenAI
      let agent = case createAgent "my_agent" model Nothing of Right a -> a; Left _ -> error "Should not fail"
      agentName agent @?= "my_agent"

  , testCase "agentDescription accessor" $ do
      let model = createModel "gpt-4" OpenAI
      let agent = case createAgent "my_agent" model (Just "Description") of Right a -> a; Left _ -> error "Should not fail"
